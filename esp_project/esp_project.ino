...
#define BTN_1COFFEE_PIN 15 //D4 for the button to make 1 coffee //for esp32s3 change 
...
//store the ssid and password value through the code
char storedSSID [16];
char storedPassword [16];

//var and functions declaration
//WifiManager managerWifi;

void IRAM_ATTR handleCupDetectedInterrupt();//Interrupt function for IR cup detector
void IRAM_ATTR checkLEDStateInterrupt(); // Interrupt function to check LED state by counting the number of state changes
void vFirebasePingTimerCallback(TimerHandle_t xTimer);// Callback function to send data and the ping to Firebase
void vWaterLevelTimerCallback(TimerHandle_t xTimer);//Callback function to send waterlevel value to Firebase
void vCalculateLedFrequencyTimerCallback(TimerHandle_t xTimer);// Callback function to calculate the LED frequency
void vCoffeeTaskTimerCallback(TimerHandle_t xTimer);//Callback function to handle the coffee requests
bool isCoffeeMachineReady();//check if the coffee machine is ready to make coffee
bool doRequestedAction(String action);//action to do with the coffee machine
void transformRequest(JsonDocument macRequest);//allow to move requests in firebase (list[0] -> next -> current)
void handleCredentials(const String ssid, const String password);//Callback to handle credentials
String getDeviceName();//give the BLE name of the device
float convertWaterSensorValues(); //convert values get by wire into % of waterlevel
void WaterLevelReadingSection();//for capting the waterlevel
...
void setup() {
  ...
  //else{
    ...
    //handleCredentials("galaxya425g8288","avmw4585");
    // managerWifi= WifiManager::getInstance();
    //managerWifi.beginWifi("galaxya425g8288","avmw4585");//storedSSID,storedPassword
    //while(!managerWifi.checkConnected()){
    ...
    while (WiFi.status() != WL_CONNECTED) { // let 6s for being connected to the Wifi station
      Serial.println(".");
      delay(1000);
    }
    ...
    // attachInterrupt(digitalPinToInterrupt(LED_PIN),checkLEDStateInterrupt,CHANGE);
    // Create timers

    //for ping to firebase ("the heart of the ESP")
    xFirebasePingTimerHandle = xTimerCreate(
      "FirebasePingTimer",          // Timer name
      pdMS_TO_TICKS(4000),    // Timer period in ticks (1 minute)
      pdTRUE,                   // Auto-reload
      (void*)0,                 // Timer ID
...
//Interrupt function for IR cup detector
...
// Callback function to send data and the ping to Firebase
void vFirebasePingTimerCallback(TimerHandle_t xTimer) {
  JsonDocument deviceStatus;
  //deviceStatus["mac"] = ManagerFirebase.mac;
  ...
  Serial.printf("devices/ %s",WiFi.macAddress());
  //firebaseData.setJson("devices/" + WiFi.macAddress(),output);
...
void vCoffeeTaskTimerCallback(TimerHandle_t xTimer) {
  sonDocument macRequest = ManagerFirebase.getData("requests/", true);//get the current, the list and the next requests
  JsonDocumeJnt nextRequest;
...
//allow to move requests in firebase (list[0] -> next -> current)
